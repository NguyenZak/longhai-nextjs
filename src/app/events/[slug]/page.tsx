    ...
    <main className="bg-white text-gray-900 pt-20">
      ...
      <div className="relative w-full overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-white/30 backdrop-brightness-50" />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto p-6">
          <div className="flex flex-col md:flex-row md:items-start gap-8">
            ...
            <div className="md:w-1/2 w-full flex flex-col gap-4 p-4 text-white">
              <span
                className={`inline-block w-35 font-semibold text-sm px-4 py-1 rounded-full ${getStatusStyle(event.status)}`}
              >
                {event.status}
              </span>

              <h1 className="text-xl md:text-2xl font-bold uppercase">{event.title}</h1>
        ...
        {activeTab === 2 && (
          <div className="text-sm text-gray-700">
            ...
            {event.artists && event.artists.length > 0 ? (
              <ArtistCarousel artists={event.artists} />
            ) : (
